...
void switchHit()
{
  int pin8Value = digitalRead(rightDOWN);
  int pin7Value = digitalRead(rightUP);

  int pin10Value = digitalRead(leftUP);
  int pin13Value = digitalRead(leftDOWN);

  int pin12Value = digitalRead(toggleRIGHT);
  int pin11Value = digitalRead(toggleLEFT);
